...
export function PalletVisualizer({ placedBoxes, hoveredBoxId, setHoveredBoxId, isLoading, palletDimensions }: PalletVisualizerProps) {
  ...
  useEffect(() => {
    const onMouseMove = (event: MouseEvent) => {
        ...
        if (tooltipPosition) setTooltipPosition({ x: event.clientX, y: event.clientY }); // Update position even if same box
    };
  ...
  useEffect(() => {
    ...
    scene.background = new THREE.Color(getComputedStyle(document.documentElement).getPropertyValue('--card'));
    sceneRef.current = scene;

    // Camera
    const camera = new THREE.PerspectiveCamera(50, currentMount.clientWidth / currentMount.clientHeight, 1, 2000);
    ...
    // Pallet
    const palletGeometry = new THREE.BoxGeometry(palletDimensions.length, PALLET_HEIGHT, palletDimensions.width);
  ...
  useEffect(() => {
    ...
    if (palletMesh) {
        ...
        palletMesh.geometry = new THREE.BoxGeometry(palletDimensions.length, PALLET_HEIGHT, palletDimensions.width);

        const newGridTexture = createGridTexture(palletDimensions.width, palletDimensions.length);
    ...
    if (cameraRef.current) {
        cameraRef.current.position.set(palletDimensions.length * 1.2, palletDimensions.maxHeight * 0.75, palletDimensions.width * 1.5);
    }
  }, [palletDimensions]);
  ...
  useEffect(() => {
    ...
    placedBoxes.forEach(box => {
      ...
      const mesh = boxMeshesRef.current.get(box.id) || new THREE.Mesh(geometry, material);
      ...
      if (!boxMeshesRef.current.has(box.id)) {
        ...
        labelDiv.className = 'text-xs bg-card/80 backdrop-blur-sm text-card-foreground px-2 py-1 rounded shadow-lg';
      ...
      mesh.geometry = new THREE.BoxGeometry(box.rotatedLength, box.rotatedHeight, box.rotatedWidth);
      ...
      {isLoading && (
       ...
       {!isLoading && placedBoxes.length === 0 && (
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-background/80 backdrop-blur-sm z-10 text-center p-4">
                ...
                <p className="text-sm text-muted-foreground">Add boxes, set pallet dimensions, and click "Optimize". Hover over boxes to see details.</p>
            </div>
       )}
        {localHoveredBoxId && tooltipPosition && (() => {
            const hoveredBox = placedBoxes.find(box => box.id === localHoveredBoxId);
            ...
            return (
                ...
                >
                    {hoveredBox.length.toFixed(1)} x {hoveredBox.width.toFixed(1)} x {hoveredBox.height.toFixed(1)} cm<br/>
